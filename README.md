...
This project is fully pre-configured for **1-click deployment on [Railway](https://railway.app)** using **Nixpacks**, **Dockerfile**, or **Procfile**.

### Step-by-Step Railway Deployment:

1. **Push your repository to GitHub**
   Ensure all changes including root `package.json`, `nixpacks.toml`, and `railway.json` are committed to your GitHub repository.
...
3. **Configure Environment Variables**
   - In your Railway service dashboard, go to the **Variables** tab.
   - Add `GEMINI_API_KEY`: `your_gemini_api_key_here` *(Optional: if omitted, the application runs with an intelligent dynamic AI fallback)*.

4. **Deploy & Generate Domain**
   - Railway will automatically detect the build setup via `nixpacks.toml` or `Dockerfile`, build the React frontend, and launch the FastAPI server.
   - Go to the **Settings** tab -> **Networking** -> Click **Generate Domain**.
   - Open your live public URL!
...
docker run -p 8000:8000 -e GEMINI_API_KEY="your_api_key" ai-interview-agent
```
Access the fullstack application live at `http://localhost:8000`.
